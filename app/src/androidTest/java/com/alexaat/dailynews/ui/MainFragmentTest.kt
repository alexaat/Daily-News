...
class MainFragmentTest {
    ...
    @Test
    fun mainFragment_DisplayedInUi() = runBlockingTest {
        repository.refreshNewsArticles()
        launchFragmentInHiltContainer<MainFragment>(null, R.style.AppTheme)
        onView(withId(R.id.news_list_recyclerview)).check(matches(isDisplayed()))
        val title1 = "Chorley v Derby County: FA Cup third round – live!"
        onView(withId(R.id.news_list_recyclerview)).check(matches(hasDescendant(withText(title1))))
